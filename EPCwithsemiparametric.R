...
# Read the raw data
setwd("D:/HJ paper/semiparametric EPC_JQT/R-code")
...
# The 2nd, 14th, 16th and 17nd variables are selected and 
...
Moment = function(x, pn = 0.9, alpha = 0.0027){
  ...
  z=qnorm((1+pn)/2)
  m=max(floor(n*alpha/2),1)
  UCL1=Rx[n-m]+z*Rx[n-m]*Mn.1(m)*abs(1-min(0,r.m.final))/sqrt(m) # Theorem 5.1
  
  UCL=Rxx+Ry[n-m]+z*Ry[n-m]*Mn.1(m)*abs(1-min(0,r.m.final))/sqrt(m) # HJ test
  ...
  LCL1=-(Rx[n-m]+z*Rx[n-m]*Mn.1(m)*abs(1-min(0,r.m.final))/sqrt(m)) # Theorem 5.1
  LCL=-(Rxx+Ry[n-m]+z*Ry[n-m]*Mn.1(m)*abs(1-min(0,r.m.final))/sqrt(m)) # HJ test
